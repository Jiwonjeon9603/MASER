...
#        group_roles = []
 ...
 #           agent_outs, group_role = self.mac.forward(batch, t=t) #(bs,n,n_actions)
            agent_outs = self.mac.forward(batch, t=t) #(bs,n,n_actions)
            mac_out.append(agent_outs) #[t,(bs,n,n_actions)]
#            group_role = group_role.reshape(-1, self.n_agents, 3)
#            group_roles.append(group_role[0])

        mac_out = th.stack(mac_out, dim=1)  # Concat over time
      #  group_roles = th.stack(group_roles, dim=0)
        ...
        for t in range(batch.max_seq_length):
            #target_agent_outs, target_group_roles = self.target_mac.forward(batch, t=t) #(bs,n,n_actions)
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            #self.logger.log_stat("grouped_role", group_roles, t_env)
